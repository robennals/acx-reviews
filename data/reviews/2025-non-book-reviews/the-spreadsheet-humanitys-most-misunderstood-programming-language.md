...
In 1979 two Harvard Business School graduates released VisiCalc, the "visible calculator".[^1] It sold Apple II machines to MBAs faster than Space Invaders ever could. Over the next decade Lotus 1-2-3 and Microsoft Excel colonised finance, science and football transfer gossip alike. By 1996, an estimated 90 percent of the world's financial models lived in Excel workbooks.[^2] Even today, the rail operator Deutsche Bahn schedules rolling stock in Excel files so large that a single sheet, if printed, would stretch for three kilometres.[^3]
...
But what could possibly go wrong? Spreadsheets fail silently. A missing "$" in a cell reference is invisible to the casual reviewer yet can propagate a hundred rows downward. The London Olympics once oversold 10,000 tickets because of a single-cell mis-paste. [^4] This is the silent error syndrome.[^5] Then there is version-control purgatory. A Git merge conflict is painful; a chain of "final\_v2\_THISONE.xlsx" emails is worse. Because spreadsheets blur code and data,[^6] classical versioning tools struggle.[^7] Finally, spreadsheets contribute to the modelling-as-fact illusion. Stakeholders reading a colourful Excel dashboard often forget that every number rests on fragile assumptions. When VaR models built in spreadsheets underestimated tail risk in 2007, billions evaporated before anyone noticed.[^8]
...
So, how can we move towards safer cells? Spreadsheet aficionados propose three remedies: linting tools that flag inconsistent formulas;[^9] typed ranges that disallow text-in-numeric cells;[^10] and integration with Git-like history. All exist, none dominate, because the market rewards compatibility over discipline.
